...
function doPredict(value) {
  ...
  app.models.predict(modelID, value).then(
    function(response) {
      ...
      var predictedItem = response.rawData.outputs[0].data.concepts[0].name;
      var modelNameShort = modelName.split("-")[0];
      var modelHeader =
        '<b><span style="font-size:20px; color: #0450d4">' +
        capitalize(predictedItem) +
        "</span></b><br/><span>Predictions</span>";
      var api_end = `${endpoint}?app_id=${nutri_app_key}&app_key=${nutri_api_key}&ingr=one%20${predictedItem.replace(
        "s",
        "%20"
      )}`;
      console.log(api_end);
      fetch(api_end, {
        ...
        .then(response => {
          console.log(response);
          var totalNutrients = response.totalNutrientsKCal;
          var resultTable = document.createElement("table");
          resultTable.setAttribute("border", "1");
          var headerRow = document.createElement("tr");
          var dataRow = document.createElement("tr");
          Object.values(totalNutrients).forEach(element => {
            // adding header row
            var headerLabel = document.createElement("th");
            headerLabel.setAttribute("style", "padding:10px");
            headerLabel.innerHTML = element.label;
            headerRow.appendChild(headerLabel);
          });
          Object.values(totalNutrients).forEach(element => {
            var dataCell = document.createElement("td");
            dataCell.setAttribute("style", "padding:10px");
            dataCell.innerHTML = `${element.quantity} ${element.unit}`;
            dataRow.appendChild(dataCell);
          });
          resultTable.appendChild(headerRow);
          resultTable.appendChild(dataRow);
          var resultSections = document.getElementById("results");
          var title = document.createElement("span");
          title.innerHTML = capitalize(predictedItem);
          var br = document.createElement("br");
          title.style = "font-size: 16px;";
          resultSections.appendChild(title);
          resultSections.appendChild(resultTable);
          resultSections.appendChild(br);
          resultSections.appendChild(br);
        });
      ...
      if (response.rawData.outputs[0].data.hasOwnProperty("regions")) {
        ...
        for (var i = 0; i < regionArray.length; i++) {
          ...
          if (modelName == "demographics") {
            ...
            var ethnicArray =
              regionArray[i].data.face.multicultural_appearance.concepts;
            var genderArray =
              regionArray[i].data.face.gender_appearance.concepts;

            // Age Header
            conceptNames +=
              '<br/><b><span style="font-size:10px">Age Appearance</span></b>';
            ...
            // Ethnicity Header
            conceptNames +=
              '<b><span style="font-size:10px">Multicultural Appearance</span></b>';
            ...
            // Gender Header
            conceptNames +=
              '<b><span style="font-size:10px">Gender Appearance</span></b>';
          ...
          // For faces just print bounding boxes
            ...
            conceptNames +=
              "<li>Top Row: <i>" +
              regionArray[i].region_info.bounding_box.top_row +
              "</i></li>";
            conceptNames +=
              "<li>Left Column: <i>" +
              regionArray[i].region_info.bounding_box.left_col +
              "</i></li>";
            conceptNames +=
              "<li>Bottom Row: <i>" +
              regionArray[i].region_info.bounding_box.bottom_row +
              "</i></li>";
            conceptNames +=
              "<li>Right Column: <i>" +
              regionArray[i].region_info.bounding_box.right_col +
              "</i></li>";
          }
          ...
          // Logos
            ...
            // Print all results
            conceptNames +=
              '<br/><b><span style="font-size:10px">Logo</span></b>';
            conceptNames +=
              "<li>" +
              regionArray[i].data.concepts[0].name +
              ": <i>" +
              regionArray[i].data.concepts[0].value +
              "</i></li>";
            conceptNames +=
              '<br/><b><span style="font-size:10px">Location</span></b>';
            conceptNames +=
              "<li>Top Row: <i>" +
              regionArray[i].region_info.bounding_box.top_row +
              "</i></li>";
          ...
          // Focus
            ...
            conceptNames +=
              '<br/><b><span style="font-size:10px">Focus Region</span></b>';
            conceptNames +=
              "<li>Top Row: <i>" +
              regionArray[i].region_info.bounding_box.top_row +
              "</i></li>";
            ...
            if (i === regionArray.length - 1) {
              conceptNames +=
                "<br><br><li><br><br>Overall Focus: <i>" +
                response.rawData.outputs[0].data.focus.value +
                "</i><br><br><br></li>";
            }
          }
      ...
      conceptNames =
        '<ul style="margin-right:20px; margin-top:20px; columns:' +
        columnCount +
        "; -webkit-columns:" +
        columnCount +
        "; -moz-columns:" +
